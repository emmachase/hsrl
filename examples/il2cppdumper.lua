...
local log = { builder = {} } --io.open(DUMP_LOG_FILE, "w")
function log:write(data)
    table.insert(self.builder, data)
    hsrl.writefile(DUMP_LOG_FILE, table.concat(self.builder, ""))
end
function log:close()
    hsrl.writefile(DUMP_LOG_FILE, table.concat(self.builder, ""))
end
...
local function do_dump_csharp_field(file, field)
    ...
    file:write(string.format("; // MetadataToken: 0x%X", field.MetadataToken))
...
local function do_dump_csharp_property(file, property)
    ...
    -- file:write("}\n")
    file:write("} // MetadataToken: 0x" .. property.MetadataToken .. "\n")
end
...
local function do_dump_csharp_type(file, type, index, rvas)
    ...
    file:write(string.format("// Module: %s (%s)\n", tostring(type.Module.Name), tostring(type.Module)))
...
local file = { builder = {} } --io.open(DUMP_file_FILE, "w")
function file:write(data)
    table.insert(self.builder, data)
end
function file:close()
    hsrl.writefile(DUMP_CS_FILE, table.concat(self.builder, ""))
end
...
local function do_dump_csharp()
    ...
    for i = 0, assemblies.Length - 1 do
        local assembly = assemblies[i]
        local assemblyName = assembly:ToString()
        -- if assemblyName:match("^mscorlib") or assemblyName:match("^System") or assemblyName:match("^Unity") or assemblyName:match("^Mono") or assemblyName:match("^RPG.GameCore.FixedMath") or assemblyName:match("^RPG.GameCore.Config") then
        --     continue
        -- end

        if not assemblyName:match("^RPG%.Network%.Proto") then
            continue
        end

        log:write(assemblyName.."\n")
        -- if not assemblyName:match("^Assembly%-CSharp,") then
        --     continue
        -- end
        ...
        for j = 0, types.Length - 1 do
            ...
            -- if j < 16000 and j % 100 ~= 0 then
            --     continue
            -- end

            if j % 100 == 0 then
                log:write(string.format("dumping type %d: %s\n", j, type:ToString()))
                log:close()
            end
