...
namespace offsets {
  ...
  inline uintptr_t get_exported(const char* module, const char* name) {
    uintptr_t x = (uintptr_t)(GetProcAddress(offsets::get_module<HMODULE>(module), name));
    std::printf("Found %s at 0x%llX\n", name, x);
    return x;
  }
}
...
namespace offsets {
  ...
  inline uintptr_t luaL_checkstack = 0;
  ...
  inline uintptr_t lua_pushfstringL = 0;
